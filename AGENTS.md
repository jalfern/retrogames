...
| `scripts/heistplay.mjs` | Chrome **plays** job 1: walks with an actual thumbstick vector, steals, delivers, chews a vault, stands in a torch beam until bagged, chews a friend loose, empties the cart, escapes. Asserts the cast is a visible mesh in the scene graph (the cast once passed every numeric check while never being added to the scene), that the verb shown is the verb that fires, that every verb has a *mesh* where it points, and that a rescue takes under 4 s. **Every wait is `window.__simSleep(worldSeconds)` and every budget is `window.__gameTime()`** — the driver waits in *world* seconds, because `setTimeout` measures the runner: ten CI reds came from waiting a wall second and asserting a game second. `--throttle N` runs the whole suite on a deliberately slow CPU. It counts the *pound*, never `probe().caged` (being caught hands you the next raccoon, so the one you are watching is free again a frame after a good arrest), and it fills the pound to two itself rather than letting the rescue section loop over an empty list. |
