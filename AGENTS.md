...
npm run heistplay  # Chrome plays job 1: 107 assertions (needs `npm run dev`)
npm run heistmutate # put nine fixed bugs back and prove heistplay notices (~20 min)
...
| `scripts/heistplay.mjs` | Chrome **plays** job 1: walks with an actual thumbstick vector, steals, delivers, chews a vault, stands in a torch beam until bagged, chews a friend loose, empties the cart, escapes. Asserts the cast is a visible mesh in the scene graph (the cast once passed every numeric check while never being added to the scene), that the verb shown is the verb that fires, that every verb has a *mesh* where it points, and that a rescue takes under 4 s. |
| `scripts/heistmutate.mjs` | The mutation harness for the above: nine mutants, each one a previously fixed bug put back by an anchored text swap, each run through the real `heistplay`, each restored from a `/tmp` copy (never `git checkout` on a dirty tree). Exits non-zero if any mutant survives, i.e. if a check has quietly stopped touching the code. `--dry` re-checks the anchors. Two of its mutants are *equivalent* and documented in its header — hiding a lock with `visible = false` (reset() repairs it before play) and measuring a merged mesh by object origin (the origin sits on the hinge) — which is the record of how the harness learned what it knows. |
