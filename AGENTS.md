...
npm run heistmutate # put eleven fixed bugs back and prove heistplay notices (~30 min)
...
| `scripts/heistmutate.mjs` | The mutation harness for the above: eleven mutants (a mutant may carry `throttle: N`, because the camera-floor overshoot literally does not exist at 60 fps), each one a previously fixed bug put back by an anchored text swap, each run through the real `heistplay`, each restored from a `/tmp` copy (never `git checkout` on a dirty tree). Exits non-zero if any mutant survives, i.e. if a check has quietly stopped touching the code. `--dry` re-checks the anchors. Two of its mutants are *equivalent* and documented in its header — hiding a lock with `visible = false` (reset() repairs it before play) and measuring a merged mesh by object origin (the origin sits on the hinge) — which is the record of how the harness learned what it knows. |
